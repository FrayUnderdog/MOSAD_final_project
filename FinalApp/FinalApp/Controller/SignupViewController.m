...
-(UIButton*)button {
    if (_button == nil) {
        ...
        [_button addTarget:self action:@selector(registerClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _button;
}
...
-(void)registerClick:(UIButton*)button{
    ...
    NSURL * url = [NSURL URLWithString:@"http://172.18.176.201:3000/register"];
...
- (UIButton*)button2 {
    if (_button2 == nil) {
        ...
        NSMutableAttributedString* tncString = [[NSMutableAttributedString alloc] initWithString:@"I have an account"];
